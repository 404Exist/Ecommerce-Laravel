...
class TrademarkDatatable extends DataTable
{
    public function dataTable($query)
    {
        return datatables()
            ->eloquent($query)
            ->editColumn('name', function(Trademark $trademark) { return $trademark->getTranslation('name', lang()); })
            ->editColumn('logo', function(Trademark $trademark) {
                return !empty($trademark->logo) ? "<img src='". asset('storage/'.$trademark->logo)."' style='width: 30px;height: 30px;' />" : '';
            })
            ->editColumn('deletes', function(Trademark $trademark){$id = $trademark->id;$name = $trademark->getTranslation('name', lang());
                return '
                    <a data-toggle="modal" data-target="#multipleDelete"
                    onclick="delete_admin_modal_ui('.$id.', \''.$name.'\', \'check_trademarks\', \'deleteTrademarksForm\')"
    ...
    public function html()
    {
        return $this->builder()
                    ...
                    ->buttons(
                        Button::make()
                            ->attr(['onClick' => 'window.location.href = "/admin/trademarks/create"'])
                            ->className('btn btn-info')
                            ->text('<i class="fa fa-plus"></i> Add Trademark'),
                        Button::make()
                            ->attr(['data-toggle'=>'modal', 'data-target'=>'#multipleDelete', 'onClick' => 'delete_admin_modal_ui("", "", "check_trademarks", "deleteTrademarksForm")'])
    ...
    protected function getColumns()
    {
        return [
            ...
            Column::computed('checkbox')
                ->orderable(false)
                ->title('<input type="checkbox" class="checkall" onClick="toggleCheckAll(this, \'check_trademarks\')"/>')
                ->exportable(false)
                ->printable(false)
                ->searchable(false)
                ->addClass('text-center'),
